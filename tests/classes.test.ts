...
// fails when a CLASS ROW changes and nothing else does. `sim.test.ts` owns the turn loop
// these signatures hook into, `content.test.ts` owns the catalog rows, and
// `progression.test.ts` owns the curve the per-class HP is paid along.
//
// **It split again at Stage 6b-3**, when it crossed 400 lines: `collection.test.ts` took
// what a delver OWNS and what the six class-locked rows DO. The seam is the schedule —
// a class is three rows that almost never move, a collection is thirty-six that move
// every time the probe says the shaft is wrong.
//
// **A class stopped being draw weights at Stage 6b-3.** The Endless does not draw at all
// now — you own abilities and build a bar from what you own (`CLASSES.md` § the override)
// — so a class is exactly two things: **the rows nobody else can cast, and one number.**
// Everything in this file about weights went with the draw.
//
// **Four things this file exists to catch**, and each of them is silent otherwise:
//
//  1. **A class reaching the Daily.** `simulateRun` is two arguments and the day's draw
//     takes one, and both of those are easy to widen by accident.
//  2. **A signature that does nothing.** All three are numeric fields on a kit, and a
//     field nothing reads type-checks perfectly — so all three checks PLAY A RUN.
//  3. **A class with two signatures.** That is two classes sharing a chip, and the next
//     one would arrive with three.
//  4. **A draw weight coming back**, which would be the 6b-3 correction reversed.

import { assert, check, describe } from './helpers';
import { barFromKit, combatAfter, endlessKit, slotOfArchetype } from './policies';
...
import {
  ...
  classUnlockFlag, collectionAt, endlessKitFor, issuedKitForDay, issuedPoolForDay,
  simulateEndless, simulateRun,
  type IssuedKit, type RunChoice,
} from '../src/shared/sim';

describe('classes · locked rows, one number each, and the wall');
...
await check('A CLASS CARRIES NO DRAW WEIGHTS — the Endless does not draw', () => {
  // Stage 6b-3's deletion, pinned so it is a decision rather than a gap. A weight only
  // means something when somebody else is choosing your nine; the Endless hands you your
  // whole collection, so a lean would be a thumb on a scale nobody is standing on.
  // Putting one back is a design reversal and this is the line that says so.
  for (const row of CLASS_LIST) {
    for (const key of Object.keys(row)) {
      assert.ok(
        !/weight/i.test(key),
        `${row.id} carries \`${key}\` — draw weights were deleted with the draw`,
      );
    }
  }
});
...
await check('A CLASSLESS ENDLESS KIT carries no signature and no growth', () => {
  // `class: null` is a real state rather than a missing Warden: the Daily reads no class,
  // so a delver who has only ever played it genuinely has none. What must be true is that
  // the kit says so — no carry, no rage bonus, no tick, and the issued max HP.
  const collection = collectionAt(null, TUNING.hero.levelCap, 40);
  for (const seed of SEEDS.slice(0, 60)) {
    const kit = endlessKitFor(seed, null, TUNING.hero.levelCap, collection);
    assert.equal(kit.maxHp, issuedKitForDay(seed).maxHp, 'no class is no growth');
    assert.equal(kit.blockCarryPct, 0);
    assert.equal(kit.rageOnHitBonus, 0);
    assert.equal(kit.idleCooldownTick, 0);
  }
});
...
await check('a level is worth HP in the run, and a classless run is untouched', () => {
  ...
  const at = (classId: string | null, level: number): IssuedKit =>
    endlessKitFor(seed, classId, level, collectionAt(classId, level, 0));
  assert.equal(at('warden', 1).maxHp, base + classHpBonus('warden', 1));
  assert.ok(
    at('warden', 20).maxHp > at('warden', 1).maxHp,
    'twenty levels of Warden has to be worth something',
  );
  assert.equal(at(null, 20).maxHp, base);
});
...
await check('WARDEN — unspent block carries a fraction into the next turn', () => {
  ...
  const bar = barFromKit(kit, ['guard', 'wall', 'strike']);
  const opening = combatAfter(seed, kit, [bar]);
  const guardSlot = slotOfArchetype(kit, bar, 'guard');
  const after = combatAfter(seed, kit, [bar, { k: 'cast', i: guardSlot }, { k: 'end' }]);
  ...
  const bareBar = barFromKit(bare, ['guard', 'wall', 'strike']);
  const bareAfter = combatAfter(seed, bare, [
    bareBar, { k: 'cast', i: slotOfArchetype(bare, bareBar, 'guard') }, { k: 'end' },
  ]);
...
await check('WARDEN — the carry stops at a DEPTH boundary, not just a turn one', () => {
  ...
  const seed = 777;
  const kit = wardenKit(seed);
  const bar = barFromKit(kit, ['guard', 'strike', 'wall']);
  const guard = slotOfArchetype(kit, bar, 'guard');
  const strike = slotOfArchetype(kit, bar, 'strike');
...
await check('HUNTER — a hit that lands on HP charges more rage than it does for anyone else', () => {
  ...
  for (const seed of SEEDS) {
    const kit = endlessKit(seed, 'hunter');
    const bar = barFromKit(kit, ['strike', 'guard', 'wall']);
    const before = combatAfter(seed, kit, [bar]);
    if (before.threat[0]?.kind !== 'attack' || before.incoming <= 0) continue;
    if (before.incoming >= before.hp) continue;

    const hunter = combatAfter(seed, kit, [bar, { k: 'end' }]);
    const bare = issuedKitForDay(seed);
    const bareBar = barFromKit(bare, ['strike', 'guard', 'wall']);
    const plain = combatAfter(seed, bare, [bareBar, { k: 'end' }]);
    assert.equal(
...
await check('ADEPT — a turn spent on nothing ticks every cooldown further', () => {
  ...
  const adept = combatAfter(seed, kit, [bar, { k: 'cast', i: slot }, { k: 'end' }, { k: 'end' }]);
  const bare = issuedKitForDay(seed);
  const bareBar = barFromKit(bare, ['burst', 'strike', 'guard']);
  const bareSlot = slotOfArchetype(bare, bareBar, 'burst');
  const plain = combatAfter(seed, bare, [bareBar, { k: 'cast', i: bareSlot }, { k: 'end' }, { k: 'end' }]);
  assert.equal(
...
await check('ADEPT — a turn that spent energy banks NOTHING extra', () => {
  ...
  for (const seed of SEEDS) {
    const kit = endlessKit(seed, 'adept');
    const bar = barFromKit(kit, ['burst', 'strike', 'guard']);
    const burst = slotOfArchetype(kit, bar, 'burst');
    const strike = slotOfArchetype(kit, bar, 'strike');
...
await check('ALL THREE ARE STARTING CLASSES — no level gate on any of them', () => {
  // **Reversed at Stage 6b-4, and the reversal is the point of the check.** Hunter opened
  // at 5 and Adept at 10 until the choice became PERMANENT — at which point the gates and
  // the decision could not both exist, because a permanent choice made on a first delve
  // against a roster of one is a stamp rather than a decision. Every delver would have been
  // a Warden forever and the other two would have been unreachable content.
  //
  // What it costs is a pacing beat in week one, and that cost is on the record in
  // `CLASSES.md` § Choosing a class rather than quietly absorbed.
  assert.ok(classById(DEFAULT_CLASS_ID), 'Warden is still the one a prompt defaults to');
  for (const row of CLASS_LIST) {
    assert.equal(row.unlockLevel, 1, `${row.id} must be available on a first delve`);
  }
  // The FIELD stays, because evolution is still a level gate and a spec will need one —
  // so this is three rows agreeing on a number, not a dead field to delete.
  assert.ok(TUNING.hero.levelCap > 1, 'and there is still a curve for a spec to sit on');
});

// ---- helpers --------------------------------------------------------------------
//
// The bar-building and kit-building helpers moved to `policies.ts` when this file split.
// They are what a test needs to PLAY a classed delver, `collection.test.ts` needs the
// same ones, and two copies of "build a legal bar" is two things to get wrong.
